            ...
            <select class="form-control select2bs4" name="dist_id" id="district" style="width: 100%;" required="required">
			  <option value="">Select District Name</option>
                  <?php foreach ($district_data as $v): ?>
                     <option value="<?php echo $v['id'] ?>" <?php if($user_data['dist_id'] == $v['id']) { echo 'selected'; } ?> ><?php echo $v['district_name'] ?> </option> 
                  <?php endforeach ?>
		    </select>
		</div>
	</div>	
		...
		<div class="col-sm-8">
            <select class="form-control select2bs4" name="panchayath_id" id="panchayath" style="width: 100%;" required="required">
                  ...
                  <?php endforeach ?>
	                ...
	                <div class="form-group row">
					<label for="referral_id" class="col-sm-4 col-form-label"><span style="color:red"></span> Referral User</label>
		<div class="col-sm-8">
						<select class="form-control select2bs4" name="referral_id" style="width: 100%;" required="required">
							<option value="<?php echo $user_data['referral_id'] ?>"><?php echo $user_data['referral_id'] ?> ReferralID</option>
								 <?php 
							   $data = $this->model_users->getUsers();
							   foreach($data as $row)
							  {  ?>
                     <option value="<?php echo $row['id'] ?>" <?php if($user_data['referral_id'] == $row['id']) { echo 'selected'; } ?> ><?php echo $row['id'] ?> --> <?php echo $row['full_name'] ?> </option> 
							<?php } ?>
						</select>
				    </div>
				</div>
              	...
              	<div class="form-group row">
					<label for="group_id" class="col-sm-4 col-form-label"><span style="color:red"></span>User Type</label>
					<div class="col-sm-8">
            <select class="form-control select2bs4" name="group_id" id="group_id" style="width: 100%;" required="required">
                    <?php foreach ($group_data as $k => $v): ?>
                     <option value="<?php echo $v['id'] ?>" <?php if($user_data['group_id'] == $v['id']) { echo 'selected'; } ?> ><?php echo $v['group_name'] ?> </option> 
                    <?php endforeach ?>
            </select>
					</div>
				</div>
<?php } if(($this->session->userdata('group_id') == '4') || ($this->session->userdata('group_id') == '6') || ($this->session->userdata('group_id') == '8') || ($this->session->userdata('group_id') == '10') || ($this->session->userdata('group_id') == '5') || ($this->session->userdata('group_id') == '7') || ($this->session->userdata('group_id') == '9') || ($this->session->userdata('group_id') == '11') || ($this->session->userdata('group_id') == '12') || ($this->session->userdata('group_id') == '13')){ ?>
    ...
    <input type="hidden" name="referral_id" value="<?php echo $user_data['referral_id'] ?>"/>
	<input type="hidden" name="dist_id" value="<?php echo $user_data['dist_id'] ?>"/>
	<input type="hidden" name="taluk_id" value="<?php echo $user_data['taluk_id'] ?>"/>
	<input type="hidden" name="panchayath_id" value="<?php echo $user_data['panchayath_id'] ?>"/>
<?php } ?>
				<div class="form-group row">
