...
<?php if($this->session->userdata('group_id') == '2'){ ?>
                <h3 class="card-title">Add Super User Presidents  </h3>
<?php } if($this->session->userdata('group_id') == '3'){ ?>
                <h3 class="card-title">Add Super User Secretarys  </h3>
<?php } ?>          </div>
          <!-- /.card-header -->
          <div class="card-body">
              <form action="<?php base_url('users/adddsuperusers') ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
...
<?php if(($this->session->userdata('group_id') == '2') || ($this->session->userdata('group_id') == '3')){ 
				   $data = $this->model_users->getDistrict();
				   foreach($data as $row)
				  {  
