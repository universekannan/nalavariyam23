...
$sql="Select * from users where dist_id = '$dist_id' and group_id='$group_ids' order by id desc limit 1 ";    
$query = $this->db->query($sql);
$datax =  $query->row();
if(!empty($datax)){ 
$assigned_user_id = $datax->assigned_user_id;
 //print_r($assigned_user_id);die;

} else{
$assigned_user_id = $one_pay;
 //print_r($assigned_user_id);die;
...
$update_data = array('log_id' => $login_id,'from_id' => $login_id,'to_id' => $one_pay,'customer_id' => $customer_url,'service_id' => $service_url,'pay_id' => $insert_id,'amount' => $amount,'ad_info' => 'Income','service_status' => 'IN Payment','time' => date('h:i:sa'),'paydate' => date('Y-m-d'));
...
//------------------
$update = array('log_id' => $login_id,'from_id' => $assigned_user_id,'to_id' => $get_user,'customer_id' => $customer_url,'service_id' => $service_url,'pay_id' => $insert_id,'amount' => 10,'ad_info' => 'Fom Work','service_status' => 'Out Payment','time' => date('h:i:sa'),'paydate' => date('Y-m-d'));
//print_r($update);die;

$insert = $this->db->insert('payment', $update);

$update_data = array('log_id' => $login_id,'from_id' => $get_user,'to_id' => $assigned_user_id,'customer_id' => $customer_url,'service_id' => $service_url,'pay_id' => $insert_id,'amount' => 10,'ad_info' => 'Income','service_status' => 'IN Payment','time' => date('h:i:sa'),'paydate' => date('Y-m-d'));
 //print_r($update_data);die;
...
if ($query->num_rows == 1) {
$user_amount = $Service_data['service_payment'];
}
//print_r($user_amount);die;


$update_data = array('log_id' => $login_id,'from_id' => $login_id,'to_id' => $customer,'customer_id' => $customer_url,'service_id' => $service_url,'pay_id' => $insert_id,'amount' => $user_amount,'ad_info' => 'Income','service_status' => 'IN Payment','time' => date('h:i:sa'),'paydate' => date('Y-m-d'));

$insert = $this->db->insert('payment', $update_data);

$update = array('log_id' => $login_id,'from_id' => $customer,'to_id' => $login_id,'customer_id' => $customer_url,'service_id' => $service_url,'pay_id' => $insert_id,'amount' => $user_amount,'ad_info' => 'Fom Work','service_status' => 'Out Payment','time' => date('h:i:sa'),'paydate' => date('Y-m-d'));
...
if ($query->num_rows == 2) {
$userpay = $Service_data['service_payment'] /2 ;
$user_amount = (round($userpay));
...
if ($query->num_rows == 2) {
 if(($this->session->userdata('taluk_id') == $this->input->post('taluk_id') )) {
$userpay = $Service_data['service_payment'] / 4 * 3 /2;
$user_amount = (round($userpay));
...
$pai = (round($pay));
 ...
 if(($this->session->userdata('taluk_id') == $this->input->post('taluk_id') )) {
$userpay = $Service_data['service_payment'] / 4;
$user_amount = (round($userpay));

} else{
$userpay = $Service_data['service_payment'] / 3;
$user_amount = (round($userpay));
...
$pay = $Service_data['service_payment'] / 2;
$pai = (round($pay));
}
...
$user_amount = $Service_data['service_payment'] / 2;
}
}
...
$user_amount = $Service_data['service_payment'] / 3;
} if ($query->num_rows == 4) {
$user_amount = $Service_data['service_payment'] / 4;
}
