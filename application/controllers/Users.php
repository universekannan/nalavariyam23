...
public function create()
	{
		...
		$this->form_validation->set_rules('aadhaar_no', 'Aadhaar No', 'trim|required|is_unique[users.aadhaar_no]');
        if ($this->form_validation->run() == TRUE) {
        	...
        	$data = array(
				...
				'profile_photo' => site_url().'assets/dist/img/user.jpg',
        	);
	...
	public function edit()
	{
		...
		//print_r($data);die;
			...
			if(!empty($_FILES['photo']['name']))
					...
					if($_FILES['photo']['size']!=0)
					{
						if(is_uploaded_file($_FILES['photo']['tmp_name'])) 
						{
							...
							if (move_uploaded_file($_FILES['photo']['tmp_name'], $newTempName)) 
							{
								...
								$update_data=array('profile_photo' =>$photo_name);
				...
				{
					if($_FILES['payment_qr']['size']!=0)
					{
						if(is_uploaded_file($_FILES['payment_qr']['tmp_name'])) 
						{
							$uploaddir = FCPATH.'assets/upload/payment_qr/';
						  
							$image_details = getimagesize($_FILES['payment_qr']['tmp_name']);
							
							$image_extension = image_type_to_extension($image_details[2]);
			  
							$image_name = rand(10, 99);
							
							$newFileName =  'payment_qr_oode_' . $image_name . '' . date("siGdmy").''.$image_extension;
							
							$newTempName = $uploaddir . $newFileName;                 
							
							if (move_uploaded_file($_FILES['payment_qr']['tmp_name'], $newTempName)) 
							{
								$photo_name = site_url().'assets/upload/payment_qr/'.$newFileName;
	         ...
	         $update = $this->model_users->update('users',$data,'id',$id);
				...
				$groups =  $this->uri->segment(3);
                 if($groups == '1') {
					...
					redirect('users/superusers', 'refresh');
				 } if($groups == '17') {
					redirect('users/superusers', 'refresh');
		         }
		        	}
...
if($this->session->userdata('group_id') == '2'){ 
	...
	public function adddsuperusers()
	{
		if(!in_array('viewUser', $this->permission)) {
			redirect('dashboard', 'refresh');
		}
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($create == true) {
        		...
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('users/adddsuperusers', 'refresh');
        	}
        }
        else {
            // false case
			$this->data['district_data'] = $this->model_users->getDistrict(0); 
            $this->render_template('users/add-superusers', $this->data);
        }	
	}
...
if($this->session->userdata('group_id') == '2'){ 
	    ...
	    $this->data['data'] = $this->model_users->viewsuperusers($group_id);
		$this->render_templates('users/view-superusers', $this->data);
	} 
...
if(($this->session->userdata('group_id') == '2') || ($this->session->userdata('group_id') == '16')){ 
		$group_id = '4';
} if(($this->session->userdata('group_id') == '3') || ($this->session->userdata('group_id') == '17')){ 
        $group_id = '5';
 }
 
	    $this->data['data'] = $this->model_users->viewassigneddistrictusers($group_id);

	    $this->data['datas'] = $this->model_users->viewdistrictusers($group_id);
		$this->render_templates('users/view-assigned', $this->data);
	} 
...
public function addassigneduser()
	{
		...
		if($id =  $this->input->post('district_user_id') ) {

		$this->form_validation->set_rules('district_user_id', 'district_user_id', 'trim|required|is_unique[assigned_district.district_user_id]');
        if ($this->form_validation->run() == TRUE) {
        ...
        $user = $this->input->post('user_id');
        	...
        	$data = array(
        		'district_user_id' =>  $this->input->post('district_user_id'),
        		'user_id ' => $this->input->post('user_id'),
        	);
					//print_r($data);die;
			 $insert = $this->db->insert('assigned_district', $data);
			 $data = array(
        		     'assigned_user_id' => $this->input->post('user_id'),
		        	);

	         $update = $this->model_users->update('users',$data,'id',$id);
        	if($insert == true) {
        		$this->session->set_flashdata('success', 'Successfully created');
					redirect('users/assigned/'.$group.'/'.$user, 'refresh');
        	}
        	else {
        		$this->session->set_flashdata('errors', 'Error occurred!!');
        		redirect('users/create', 'refresh');
            ...
            // false case
			$this->data['district_data'] = $this->model_users->getDistrict(0); 
            $this->render_template('users/view-assigned', $this->data);
        }	
		}
	}
...
public function viewdistrictassignedusers()
	{
	    ...
	    $this->data['data'] = $this->model_users->viewdistrictassignedusers();
		$this->render_templates('users/view-district-assigned-user', $this->data);
	} 
			...
			if(!empty($_FILES['photo']['name']))
				{
					if($_FILES['photo']['size']!=0)
					{
						if(is_uploaded_file($_FILES['photo']['tmp_name'])) 
						{
							...
							$newFileName =  'profile_photo_' . $image_name . '' . date("siGdmy").''.$image_extension;
...
public function change_pwd()
	...
	public function kannan()
	{
		...
		if(empty($this->session->userdata('id') == '2')) 
		{
			redirect('dashboard', 'refresh');
		}
		$this->render_templates('users/kannan', $this->data);
	}
	public function editkannan()
	{
		if(empty($this->session->userdata('id') == '2')){
			redirect('dashboard', 'refresh');
		}
...
$insert = $this->db->insert('payments', $data);
		$datas = array(
		  ...
		  'paydate'         => date('Y-m-d')
				...
				//print_r($data);die;
	         $update = $this->model_users->bulkpurchase('payment',$datas,'id',$id);
	        	if($insert == true) {
	        		$this->session->set_flashdata('success', 'Successfully updated');
	        		redirect('users/kannan', 'refresh');
	        	}
	        	else {
	        		$this->session->set_flashdata('errors', 'Error occurred!!');
	        		redirect('users/editkannan/'.$id, 'refresh');
	        	}
	        }
	        else {
				$this->render_template('users/editkannan', $this->data);	
	        }
	        }
		...
		public function single()
	{
       		...
       		$this->load->view('users/single', $this->data);
