...
class Wallet extends Admin_Controller 
{
	...
	public function payment()
	{
	    ...
	    $this->data['data'] = $this->model_payments->send_data_one_all('payment');
				//print_r($customer_id);die;

		$this->render_templates('wallet/all-payment', $this->data);
	}
	...
	public function paymentdelete($id)
	{
		...
		if($id) {
			if($this->input->post('confirm')) {
					$delete = $this->model_payments->paymentdelete($id);
					$delete = $this->model_payments->paymentdeletes($id);

					//print_r($delete);die;
					if($delete == true) {
		        		$this->session->set_flashdata('success', 'Successfully removed');
		        		redirect('wallet/payment', 'refresh');
		        	}
		        	else {
		        		$this->session->set_flashdata('error', 'Error occurred!!');
		        		redirect('wallet/delete/'.$id, 'refresh');
		        	}
			}	
			else {
				$this->data['id'] = $id;
				$this->render_template('wallet/delete', $this->data);
			}	
		}
	}
