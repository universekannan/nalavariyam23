...
$group_ids = '14';
		$this->data['data'] = $this->model_report->send_data_two('payments','customer_group_id',$group_ids,'service_status',$service_status);
} if($this->session->userdata('group_id') == '3'){
$group_ids = '15';
		$this->data['data'] = $this->model_report->send_data_two('payments','customer_group_id',$group_ids,'service_status',$service_status);
} if($this->session->userdata('group_id') == '4'){
$group_ids = '14';
		$this->data['data'] = $this->model_report->send_data_thre('payments','customer_group_id',$group_ids,'service_status',$service_status,'dist_id',$dist_id);
...
$group_ids = '15';
		...
		public function receipt($id = null)
	{
		if(!in_array('AllServices', $this->permission)) {
		 ...
		 $service_status= 'Img';
		$this->data['data'] = $this->model_report->send_data_two('payments','log_id',$log_id,'service_status',$service_status);
		$this->render_output('reports/receipt', $this->data);
	}
...
if(($this->session->userdata('group_id') == '2') || ($this->session->userdata('group_id') == '3')){
	   $this->data['data'] = $this->model_report->approve($customer_group_id);
	...
	public function pendingappeals()
	{
		...
		$reports_completed_data = $this->model_report->getReportsDatas($UserID);
		$this->data['reports_completed_data'] = $reports_completed_data;
...
public function tests($id = null)
	   {
		...
		$groups = $this->model_users->getUserGroup($id);
		$this->data['user_data'] = $user_data;
		$this->render_template('reports/test', $this->data);
	}
	...
	public function test($id = null)
	{
		...
		$servicedata = $this->model_service->getServiceFile($id);
		$this->data['servicedata'] = $servicedata;
		$user_data = $this->model_users->getAdninData($distID);
		$this->data['user_data'] = $user_data;
