...
class Model_service extends CI_Model
{
		...
		public function getServiceOderData($id = null) 
	{
		if($id) {
	   $status ='Completed';
	   $this->db->select("order_service.*,service.service_name");
	   $this->db->from("order_service");
	   $this->db->join("service", "order_service.service_id=service.id");
	   ...
	   return $res;
		...
		public function getBullkOderData()
	{	
       ...
       $this->db->where('confirm_order.to_id',$user_id);
